...
class Select {
  ...
  get value() {
    return this._idx >= 0 && this.options[this._idx] ? this.options[this._idx].value : "";
  }
  set value(v) {
  ...
  labels() {
    return this.options.map((o) => o.textContent);
  }
}
...
check(
  "baseline list built",
  program.labels().join(",") === "All (system audio),cs2.exe,Discord.exe",
  program.labels().join(","),
);
...
sandbox.document.activeElement = null;
check(
  "queued rebuild applied on blur",
  program.options.length === 4,
  String(program.options.length),
);
...
check(
  ...
  calls.some((c) => c[0] === "refresh_monitors"),
  JSON.stringify(calls),
);
calls.splice(0);
W.AR.refreshDropdown("program-select");
check(
  "opening program requests a fresh process list",
  calls.some((c) => c[0] === "refresh_programs"),
  JSON.stringify(calls),
);
calls.splice(0);
...
check(
  ...
  mono.selectedIndex === 0,
  String(mono.selectedIndex),
);
W.onMonoStateChanged(
  JSON.stringify({
    devices: ["Headphones", "CABLE Input"],
...
check(
  "each new modal backdrop closes without submitting",
